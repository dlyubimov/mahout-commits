...
//    }
    ...
    Writer w = new FileWriter("A.csv");
    try {
      SSVDTestsHelper.dumpDRM(fs, aPath, w);
    } finally {
      w.close();
    }

    w = new FileWriter("U.csv");
    try {
      SSVDTestsHelper.dumpDRM(fs, new Path(svdOutPath, "U/[^_]*"), w);
    } finally {
      w.close();
    }

    w = new FileWriter("V.csv");
    try {
      SSVDTestsHelper.dumpDRM(fs, new Path(svdOutPath, "V/[^_]*"), w);
    } finally {
      w.close();
    }
